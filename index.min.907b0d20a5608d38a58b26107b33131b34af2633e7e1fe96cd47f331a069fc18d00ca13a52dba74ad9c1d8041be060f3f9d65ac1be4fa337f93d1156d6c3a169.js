...
`}),e.add({id:24,href:"/docs/games/star-rail/anti-cheat/",title:"Anti-Cheat",description:"Article related to the HSR's anti-cheat",content:`Basic explanation #The game uses AntiCheatExpert (ACE), which is not natively supported on Linux and is designed for Windows. There is a Windows compatibility layer known as Wine which is designed to allow software written for Windows to run on Linux. But even with Wine, ACE does not work. The only way to get the game to run is to run it without ACE and use some workarounds and tricks to mimic ACE\u0026rsquo;s behaviour.
The game also requires DirectX shared resources, which will probably never be fully supported by DXVK/WineD3D. WineD3D is Wine\u0026rsquo;s implementation of DirectX, translating Direct3D and DirectDraw API calls into OpenGL. DXVK translates DirectX API calls to Vulkan for use with Wine. To allow the game to run, this feature must be forcibly disabled in the game\u0026rsquo;s engine through heavy engineering. This may lead to some graphical glitches.
...
astra (obsolete, see First ban wave) jadeite ToS violation #Using third-party software such as patches and external launchers is illegal according to the Company\u0026rsquo;s Terms of Service. Therefore, you may receive a ban. It is recommended that you use a burner account for the purpose of running the game on Linux.
Use at your own risk and only if you understand all the possible consequences.
History of ban waves #First ban wave #At March 25th, 2023, 10:52 AM UTC+8, players on the Asia server who played using the astra patch on Linux received a ban on their accounts for \u0026ldquo;use of third party software\u0026rdquo;. As most accounts were first offenders, they were given one week suspension until June 1st. Later, players from Europe (UTC+1) and America (UTC-5) servers received their bans at their respective local time. A well-known American Linux user also reported on this situation.
...
`}),e.add({id:41,href:"/docs/launcher/enhancements/sandbox/",title:"Sandbox",description:`Where to find #Open the launcher settings Go to the \u0026ldquo;Enhancements\u0026rdquo; tab Open the \u0026ldquo;Sandbox\u0026rdquo; sub-tab Description #In this tab you can configure the launch of the game inside the bubblewrap sandbox. A similar technique is used in the Flatpak sandbox.
This option will allow you to prevent the game access to the user\u0026rsquo;s personal files. The same happens when you use the launcher in Flatpak format, so this option will be interesting only to people who use the native launcher installation format - RPM, AUR and so on.`,content:`Where to find #Open the launcher settings Go to the \u0026ldquo;Enhancements\u0026rdquo; tab Open the \u0026ldquo;Sandbox\u0026rdquo; sub-tab Description #In this tab you can configure the launch of the game inside the bubblewrap sandbox. A similar technique is used in the Flatpak sandbox.
This option will allow you to prevent the game access to the user\u0026rsquo;s personal files. The same happens when you use the launcher in Flatpak format, so this option will be interesting only to people who use the native launcher installation format - RPM, AUR and so on.
Limitations #It is known that now the game does not have access to the pipewire/pulsaudio sound session, so you will not be able to hear the sounds inside the game. This will be fixed in future updates. `}),e.add({id:42,href:"/docs/launcher/enhancements/environment/",title:"Environment",description:`Where to find #Open the launcher settings Go to the \u0026ldquo;Enhancements\u0026rdquo; tab Open the \u0026ldquo;Environment\u0026rdquo; sub-tab Description #The environment settings allow you to change the startup command of the game and to specify environment variables. They can be useful for advanced users who are sure of what they are doing.
Environment variables and game command have special \u0026ldquo;keywords\u0026rdquo; which will be replaced by other values before starting the game - for example the word %command% will be replaced by the game starting command.`,content:`Where to find #Open the launcher settings Go to the \u0026ldquo;Enhancements\u0026rdquo; tab Open the \u0026ldquo;Environment\u0026rdquo; sub-tab Description #The environment settings allow you to change the startup command of the game and to specify environment variables. They can be useful for advanced users who are sure of what they are doing.
Environment variables and game command have special \u0026ldquo;keywords\u0026rdquo; which will be replaced by other values before starting the game - for example the word %command% will be replaced by the game starting command. You can use these to create more complex commands.
Available keywords #Common #The keywords available both in the game command field and in the values of environment variables.
Keyword Value Example %launcher% Path to the launcher folder where its data is stored \$HOME/.local/share/anime-game-launcher %build% Path to Wine build (where folders bin, lib and so on are stored) \$HOME/.local/share/anime-game-launcher/runners/lutris-GE-Proton8-7-x86_64 %prefix% Path to Wine prefix \$HOME/.local/share/anime-game-launcher/prefix %game% Path to the folder where the game is installed \$HOME/.local/share/anime-game-launcher/game %temp% Path to the temporary directory used by the luncher \$HOME/.local/share/anime-game-launcher Game launch command #Keywords available only in the field with the game launch command.
Keyword Value Example %bash_command% The part of the game starting command containing the Wine executable file, as well as additional utilities like gamemoderun gamemoderun 'HOME/.local/share/anime-game-launcher/runners/lutris-GE-Proton8-7-x86_64/bin/wine64' %windows_command% The part of the game starting command containing the Windows parameters. Mainly the path to the .bat file which runs the game and the -window-mode exclusive or -screen-fullscreen 0 -popupwindow parameters if the corresponding settings are selected launcher.bat -window-mode exclusive %command% Full command to start the game %bash_command% %windows_command% The final command, which executes the launch of the game, is built in the format bash -c \u0026quot;%command%\u0026quot;. It can be seen in the file debug.log - as well as the list of environment variables used to start the game.
You can use built-in Wine utilities to test the game\u0026rsquo;s start command - for example, specify %bash_command% winecfg as a start command. This way you can quickly find errors in the command and not wait for the launch of the game itself.
...
`}),e.add({id:48,href:"/docs/contributing/documentation/",title:"Documentation",description:"How to contribute to the documentation",content:`Getting started #For contributing you will need the following:
